...
def create_vectordb():
    ...
    vectordb = Chroma.from_documents(
        documents=texts,
        embedding=embedding,
        persist_directory=f"./{DB_NAME}"  # DB를 현재 폴더에 저장
    )
...
def create_chain():
    ...
    template = """You are a helpful assistant that can answer questions about the given context.
    Context: {context}
    Question: {question}
    Answer:
    """
...
def benchmark(chain):
    ...
    ds = load_dataset("TheFinAI/en-acronym", token=os.getenv("HF_TOKEN"))
